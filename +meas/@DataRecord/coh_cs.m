function out = coh_cs(dataRecords)
%COH_CS Computes and outputs coherence.
%   Calculate predicted and measured ANR performance from CPSD measurements
%   and digital filter specifications.
%
%   COH_CS(DATARECORDS) returns a 1xN array of structs with the fields
...
%   coh_cs's that can be calculated from the corresponding DataRecord. If
%   there are multiple possible coh_cs's from a single DataRecord,
...
x_signal = bose.cnc.meas.SignalType.FeedbackMic;
    ...
    if singleDataRec.StepType == metricStepType
        ...
        for i_y = 1:length(indc_y)
            ...
            for i_x = 1:length(indc_x)
                ...
                out(i_dataRec).data(:,i_sig) = abs(SYX).^2./SYY./SXX;
