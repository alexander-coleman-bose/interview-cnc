function out = rcr_active(dataRecords)
%RCR_ACTIVE Computes and outputs Rcr_active.
%   Calculate predicted and measured ANR performance from CPSD measurements
%   and digital filter specifications.
%
%   RCR_ACTIVE(DATARECORDS) returns a 1xN array of structs with the fields
...
%   rcr_active's that can be calculated from the corresponding DataRecord. If
%   there are multiple possible rcr_active's from a single DataRecord,
...
x_signal = bose.cnc.meas.SignalType.ReferenceMic;
metricStepType = bose.cnc.meas.StepType.NoiseActive;
    ...
    if singleDataRec.StepType == metricStepType
        ...
        for i_y = 1:length(indc_y)
            ...
            for i_x = 1:length(indc_x)
                ...
                % Math
                SYY = dataRecords(i_dataRec).getXsData(:,index_y,index_y);
                ...
                out(i_dataRec).data(:,i_sig) = SYY./SXX;
