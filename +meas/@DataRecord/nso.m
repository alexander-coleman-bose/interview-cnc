function out = nso(dataRecords)
%NSO Computes and outputs Nso.
%   Calculate predicted and measured ANR performance from CPSD measurements
%   and digital filter specifications.
%
%   NSO(DATARECORDS) returns a 1xN array of structs with the fields
...
%   nso's that can be calculated from the corresponding DataRecord. If
%   there are multiple possible nso's from a single DataRecord,
...
for i_dataRec = 1:numel(dataRecords)
    ...
    if singleDataRec.StepType == metricStepType
        ...
        if sum(mask_sig_y) == 0
            error( ...
                [msgIdHeader ':NoSignal'],...
                'No SignalType "%s" found in DataRecord(%d).', ...
                y_signal, ...
                i_dataRec ...
            );
        end
