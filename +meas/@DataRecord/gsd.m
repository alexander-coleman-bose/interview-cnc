function out = gsd(dataRecords)
%GSD Computes and outputs Gsd.
%   Calculate predicted and measured ANR performance from CPSD measurements
%   and digital filter specifications.
%
%   GSD(DATARECORDS) returns a 1xN array of structs with the fields
...
%   gsd's that can be calculated from the corresponding DataRecord. If
%   there are multiple possible gsd's from a single DataRecord,
...
measName = "gsd";
y_signal = bose.cnc.meas.SignalType.FeedbackMic;
x_signal = bose.cnc.meas.SignalType.DriverLoopback;
metricStepType = bose.cnc.meas.StepType.Driver;
