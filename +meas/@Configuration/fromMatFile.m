function obj = fromMatFile(varargin)
    %FROMMATFILE Loads all Configurations from a given file pattern.
    %
    %Usage:
    ...
    defaultPathSpec = fullfile('**', 'Configuration*.mat'); % All *.mat files, recursive.
    ...
    for indFile = 1:numel(uniqueFiles)
        try
            loadStruct = load(uniqueFiles(indFile));
